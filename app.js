...
function renderQuestionList() {
  ...
  filtered.forEach((question) => {
    ...
    const answersHtml = ["A", "B", "C", "D"].map((letter) => {
      ...
      const correctClass = question.correct === letter ? "correct-mini" : "";
    ...
    item.innerHTML = `
      <div class="question-meta">
        ...
        <span class="pill">Korrekt: ${escapeHtml(question.correct || "–")}</span>
      </div>
...
function renderAnswers(currentQuestion) {
  ...
  ["A", "B", "C", "D"].forEach((letter) => {
    ...
    button.addEventListener("click", () => {
      if (session.mode === "learn" && session.checked) return;
...
function renderFeedback(currentQuestion) {
  ...
  const isCorrect = session.selectedAnswer === currentQuestion.correct;

  box.className = `feedback-box ${isCorrect ? "good" : "bad"}`;
  box.innerHTML = isCorrect
    ? "Richtig."
    : `Falsch. Richtige Antwort: ${escapeHtml(currentQuestion.correct)}.`;
}
...
function renderQuizButtons() {
  ...
  $("check-answer-btn").disabled = !session.selectedAnswer || session.checked;
...
function nextQuestion() {
  ...
  if (session.mode === "learn" && !session.checked) {
    alert("Bitte zuerst auf „Antwort prüfen“ klicken.");
    return;
  }
